...
export default function App() {
  ...
  const dueTodos = useMemo(
    () => todos.filter((todo) => !todo.completed).slice().sort(sortDueTodos),
    [todos]
  );
  const completedTodos = useMemo(
    () => todos.filter((todo) => todo.completed).slice().sort(sortCompletedTodos),
    [todos]
  );
  ...
  return (
    <main className="container">
      <section className="app-shell">
        <header>
          <h1>Todo List</h1>
          <p>Simple React + Vite demo with due dates and completion tracking.</p>
        </header>
        ...
        {activeTab === "completed" && completedTodos.length > 0 && (
          <footer className="actions-row">
            <button type="button" className="outline" onClick={clearCompleted}>
              Clear completed
            </button>
          </footer>
        )}
